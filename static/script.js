...
function validate_login_form(form, data) {
    if (data.user_email == "") {
        ...
        addFormError(form["user_email"], 'Digita tu correo');
        return false; // stop the script if validation is triggered
    }

    if (data.user_email !== "@") {
        // if username variable is empty
        addFormError(form["user_email"], 'Debe tener @');
        return false; // stop the script if validation is triggered
    }

    if (data.user_password == "") {
        // if password variable is empty
        addFormError(form["user_password"], 'La contraseña es incorrecta');
        return false; // stop the script if validation is triggered
    }
...
function validate_registration_form(form, data) {
    if (data.user_username == "") {
        // if username variable is empty
        addFormError(form["user_username"], 'Digita tu nombre');
        return false; // stop the script if validation is triggered
    }

    if (data.user_lastname == "") {
        // if username variable is empty
        addFormError(form["user_lastname"], 'Digita tu Apellido');
        return false; // stop the script if validation is triggered
    }
    ...
    if (data.user_password == "") {
        // if password variable is empty
        addFormError(form["user_password"], 'Digita tu contraseña');
        return false; // stop the script if validation is triggered
    }

    if (data.user_cnf_password == "" || data.user_password != data.user_cnf_password) {
        // if password variable is empty
        addFormError(form["user_cnf_password"], "Las contraseñas no coinciden");
        return false; // stop the script if validation is triggered
    }

    if (!data.user_terms) {
        // if password variable is empty
        addFormError(form["user_terms"], "Debes aceptar los terminos y condiciones para registrarte");
        return false; // stop the script if validation is triggered
    }
...
$('#successful_login,#successful_registration').on('click', 'a.dialog-reset', function() {
    ...
    document.getElementById('recu_form').reset();
